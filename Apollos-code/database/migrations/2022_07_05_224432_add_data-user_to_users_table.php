...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('users', function (Blueprint $table) {
            ...
            $table->enum('rol', ['artist', 'user', 'admin'])->default('user')->after('last_name'); // Rol
            $table->string('username', 25)->after('email'); // Varchar 
            $table->enum('status', ['active', 'inactive'])->default('active')->after('username'); // Estado de cuenta
            $table->float('age', 3, 0)->after('status'); // Se calcula en base a la fecha de nacimiento y la fecha actual
