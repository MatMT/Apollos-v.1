    ...
    <footer class="">
        <div class="player activer" id="player">
            ...
            <div class="cancion">
                ...
                <img src="{{ asset('storage') . '/uploads/imagenes/' . $actuallysong->image }}" alt="Canción"
                    class="">
            </div>
            <div class="info-cancion">
                <h2 class="font-cuerpo font-bold text-base ml-1 lg:text-lg">{{ $actuallysong->name_song }}</h2>
                <a href="{{ route('profile.index', ['user' => $actuallysong->InfoArtista($actuallysong)]) }}"
            ...
            <div class="controles relative">
                ...
                <div class="other-icons rounded-md hidden anim2 {{ auth()->user()->name == $user->name ? 'other-icons-' : '' }}{{ $actuallysong->sencillo ? 'single' : 'song' }}
                    "
                    id="other-icons">
                    @if (auth()->user()->name == $user->name)
                        <form
                            action="{{ route('song.settings.index', ['user' => $user, 'song' => $actuallysong->id]) }}">
                            <input type="submit" value="{{ __('Edit song') }}" name="" id=""
                                class="transition-colors cursor-pointer mb-1 hover:text-blue-400">
                        </form>
                        ...
                        @endif
                    @endif
                    <form action="{{ route('report.mail.store', ['user' => $user, 'song' => $actuallysong->id]) }}"
